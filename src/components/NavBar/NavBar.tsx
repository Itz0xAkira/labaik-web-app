    ...
    return (
        <div id="tab-bar" className=" absolute bottom-0 left-0 w-full
         h-[10%] rounded-tr-[4rem] flex justify-between items-center px-5">
            ...
            <NavBarButton text={i18n.t("tabBar.settings")} icon='pi pi-cog' onClick={() => { onSettingsClick() }} />
            <NavBarButton text={i18n.t("tabBar.digitalIdentity")} icon='pi pi-qrcode' onClick={() => { push("digitalID") }} />
            <HomeButton />
            <NavBarButton text={i18n.t("tabBar.schedule")} icon='pi pi-calendar' onClick={() => { push("schedule") }} />
            <NavBarButton text={i18n.t("tabBar.map")} icon='pi pi-map-marker' onClick={() => { push("maps") }} />
        </div>
    )
}
